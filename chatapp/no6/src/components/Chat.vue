...
const onPublish = () => {
  if(!canPublish()) {
    alert(`1分経ってから投稿してください。(残り${60 - Math.floor((currentTime.value - lastPublishTime.value)/1000)}秒)`)
    return
  }
...
const canPublish = () => {
  ...
  if(lastPublishTime.value === null | currentTime.value - lastPublishTime.value > 60000) {
    lastPublishTime.value = currentTime.value
    return true
  }
  return false
}
