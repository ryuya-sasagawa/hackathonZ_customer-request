...
// 投稿メッセージをサーバに送信する
const onPublish = () => {
  const lastPublish = chatList[chatList.length - 1]

  if(lastPublish && isMyPublish(lastPublish)) {
    alert("同じユーザからの連続した投稿はできません。")
    return
  }
...
// 自分の投稿か判定
const isMyPublish = (publish) => {
  const index = publish.indexOf("さん")
  const name = publish.substring(0, index)
