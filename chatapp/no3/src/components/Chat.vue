...
const onPublish = () => {
  ...
  socket.emit("publishEvent", `${userName.value}さん：${chatContent.value} (${date})`)
  // 入力欄を初期化
  chatContent.value = ""
}
...
const onMemo = () => {
  const date = getDate()
  // メモの内容を表示
  chatList.push(`${userName.value}さんのメモ：${chatContent.value} (${date})`)
  // 入力欄を初期化
  chatContent.value = ""
}
// #endregion
...
const getDate = () => {
  ...
  return `${year}年${month}月${day}日 ${hour}時${minute}分${sec}秒`
}
// #endregion
</script>
